...
class Continuous_PrePosAttCov_InHiGNSSPosCov(baseEnv):
    """
    连续动作的状态预测和协方差预测（该环境只预测位置），输入新息 历史序列 GNSS特征 位置状态, 输出位置 姿态预测
    创建人：Jianhao
    修改：260528: def step() 重置RL状态方式修改
    """
    metadata = {'render.modes': ['human']}
    def __init__(self, config, **kwargs):
        # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        ...
        # TODO: 确认 ino和 his 的状态维度
        self.State_Dim = 6
        self.pos_weight = config["env_para"].get('pos_weight', 1)
        self.vel_weight = 0
    ...
    def _next_observation(self):
        ...
        Pitch_seq = self.baseline.loc[self.current_step - seq_len: self.current_step, 'Pitch_RLpredict'].values[indices]
        Heading_seq = self.baseline.loc[self.current_step - seq_len: self.current_step, 'Heading_RLpredict'].values[indices]
        obs = np.array([Lat_seq, Lon_seq, Hei_seq, Roll_seq, Pitch_seq, Heading_seq])
...
class Continuous_PrePosCov_PosAttCorrectCov_InHiGNSSSatCov(baseEnv):
    # 连续动作的状态预测和协方差预测（该环境只预测位置），输入新息 历史序列 GNSS特征 状态, 输出位置预测
    metadata = {'render.modes': ['human']}
    def __init__(self, config_agent_1, config_agent_2, **kwargs):
        # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(Continuous_PrePosCov_PosAttCorrectCov_InHiGNSSSatCov, self).__init__(config_agent_1,**kwargs)
        # TODO: 确认 ino和 his 的状态维度
        self.State_Dim = 6
        self.pos_weight = config_agent_1["env_para"].get('pos_weight', 1)
        self.threshold = chi2.ppf(0.999, self.State_Dim)
        self.h_scale = config_agent_1.get("env_para", {}).get("h_scale", 0.01)
        self.n_sigma = config_agent_1.get("env_para", {}).get("n_sigma", 3.0)
        self.observation_space = spaces.Dict(
            {'gnss': spaces.Box(low=-1, high=1, shape=(1, 10)),
             'innovation': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float),
             'History': spaces.Box(low=0, high=1, shape=(1, (self.pos_num-1) * self.State_Dim), dtype=np.float),
        ...
        self.pred_att_scale = config_agent_1["env_para"]['continuous_scale_att_pred']
        # 设置动作尺度(A2)，两智能体一些设置可能不同
        ...
        self.correct_att_scale = config_agent_2["env_para"]['continuous_scale_att_pred']
        self.prdcov_mode_A2 =  config_agent_2["env_para"]['prdcov_mode']
    ...
    def step(self, action):  # modified in 3.3
        ...
        self.KRLF_Process_test() # self.KRLF_Process(dx_rl,pred_cov_rl)
        self.fusing = False # 融合后设为False
        self.current_step = self.current_step + 1 # 当前步加1
        self.fusing_count = self.fusing_count + 1 # 融合次数加1
...
class Continuous_PrePosCov_PosCorrectCov_InHiGNSSSatCov(baseEnv):
    ...
    def step(self, action):  # modified in 3.3
        ...
        # 位置 P 调整
        cov_kf_pva = self.giengine.Cov_[StateID.P_ID:StateID.P_ID + 3, StateID.P_ID:StateID.P_ID + 3].copy()
        d_cov_rl = np.diag(self.policy_cov_scale * action[0, 3 * self.State_Dim:3 * self.State_Dim+3])
        # TODO 协方差预测不当会造成融合后对角线变为0
        if self.prdcov_mode_A2 == "Add_mode1":
            cov_kf_pva = cov_kf_pva + d_cov_rl
            cov_kf_pva[cov_kf_pva < 1e-1] = 1e-1
...
class Continuous_CorrectPosAttCov_InHiGNSSPosCov(baseEnv):
    ...
    def _next_observation(self):
        ...
        while not self.fusing: # 当开始融合时，不会再进入此循环
            ...
            navstate = self.giengine.getNavState()
            multipliers = np.array([Angle.R2D, Angle.R2D, 1.0, 1.0, 1.0, 1.0, Angle.R2D, Angle.R2D, Angle.R2D])
            # 提取 navstate 的值拼接成 1D 数组
            raw_state = np.concatenate((navstate.pos, navstate.vel, navstate.euler))
            # 一次性乘法和 round
            result = np.round(raw_state * multipliers, 9)
            nav_result.append(result)
    ...
    def step(self, action):  # modified in 3.3
        ...
        if self.done:
            obs = []
            cols_to_clear = ['Latitude_RLpredict', 'Longitude_RLpredict', 'Ellipsoid_Height_RLpredict',
                             'Velocity_X_RLpredict', 'Velocity_Y_RLpredict', 'Velocity_Z_RLpredict',
                             'Roll_RLpredict', 'Pitch_RLpredict', 'Heading_RLpredict']
            df = data_truth_dic[self.tripIDlist[self.tripIDnum]]
            df.loc[self.current_step + 1:, cols_to_clear] = np.nan
