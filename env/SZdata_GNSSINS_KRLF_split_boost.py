...
with open(dir_path + 'env/raw_baseline_SZdata_new_split.pkl', "rb") as file:
...
class Continuous_PrePosCov_InHiGNSSSatCov(baseEnv):
    ...
    def _next_observation(self):
        ...
        obs_seq = self.baseline_buffer[indices][:, [0, 1, 2, 3, 4, 5, 6, 7, 8]]
        obs = obs_seq.T
    ...
    def step(self, action):  # modified in 3.3
        ...
        rm, rn = radiusmn(blh_station[0])
        # 用于代码测试，不参与环境运行
        # gt_col = [' Latitude_GT (deg)',' Longitude_GT (deg)',' Ellipsoid Height_GT (m)']
        ...
        self.RL_prestate[0:3] = self.RL_prestate[0:3] + d_pos_llh.reshape(3) # 单位是度 测试
...
class Continuous_POSCorrectCov_InHiGNSSPosCov(baseEnv):
    # 连续动作的状态修正和协方差预测（该环境只预测位置），输入新息 历史序列 GNSS特征 状态, 输出位置预测
    metadata = {'render.modes': ['human']}
    def __init__(self, config, **kwargs):
        # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(Continuous_POSCorrectCov_InHiGNSSPosCov, self).__init__(config,**kwargs)
        # TODO: 确认 ino和 his 的状态维度
        self.State_Dim = 3
        self.pos_weight = config["env_para"]['pos_weight']
        self.observation_space = spaces.Dict(
            {'gnss': spaces.Box(low=-1, high=1, shape=(1, 10)),
             'innovation': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float),
             'History': spaces.Box(low=0, high=1, shape=(1, (self.pos_num-1) * 9), dtype=np.float),
             'State': spaces.Box(low=0, high=1, shape=(1, 3), dtype=np.float),
             'Cov': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float)})
    ...
    def _next_observation(self):
        ...
        # state feature process
        column = ['Latitude_RLpredict','Longitude_RLpredict','Ellipsoid_Height_RLpredict','Velocity_X_RLpredict','Velocity_Y_RLpredict',
                  'Velocity_Z_RLpredict','Roll_RLpredict','Pitch_RLpredict','Heading_RLpredict']
        cur_state = self.baseline.loc[self.current_step-1, column].values
        if self.cord == 'NED':  # 地理坐标系转到导航坐标系
            n,e,d = pm.geodetic2ned(cur_state[0], cur_state[1], cur_state[2], self.start_pos[0], self.start_pos[1], self.start_pos[2])
            pre_pos = np.array([n, e, d])
            pre_state = pre_pos # np.concatenate((pre_pos,cur_state[3:]))
        elif self.cord == 'LLH':
            pre_state = self.RL_prestate[0:3].copy()
        pre_state = self._normalize_state(pre_state)

        # RL cov features
        KF_P = self.giengine.Cov_[StateID.P_ID:StateID.P_ID + self.State_Dim,StateID.P_ID:StateID.P_ID + self.State_Dim].copy()
        rl_cov_diag = np.diag(KF_P) / 0.1

        # 观测合并
        obs_all = {'History': obs.reshape(1, obs.size, order='F'),
                   'gnss': obs_feature.reshape(1, 10),
                   'innovation': self.innovation.reshape(1, self.innovation.size), # 修正前后新息
                   'State': pre_state.reshape(1, pre_state.size), # 只有pos
                   'Cov': rl_cov_diag.reshape(1, rl_cov_diag.size)} # KF的P阵
    ...
    def step(self, action):  # modified in 3.3
        ...
        # gt_col = [' Latitude_GT (deg)',' Longitude_GT (deg)',' Ellipsoid Height_GT (m)']
        # gt_lat_cur = self.baseline.loc[self.current_step + 1, gt_col].values
        # gt_lat_cur[0:2] *= Angle.D2R
        # pre_pos = navstate.pos
        # d_gt = gt_lat_cur - pre_pos
        # d_pos_n = drad2dm(rm, rn, blh_station, d_gt)
        ...
        # P 调整
        cov_kf_pva = self.giengine.Cov_[StateID.P_ID:StateID.P_ID + self.State_Dim,StateID.P_ID:StateID.P_ID + self.State_Dim].copy()
        d_cov_rl = np.diag(self.policy_cov_scale * action[0, self.State_Dim:].copy())
        # TODO 协方差预测不当会造成融合后对角线变为0
        if self.prdcov_mode == "Add_mode2":
        ...
        self.giengine.Cov_[StateID.P_ID:StateID.P_ID + self.State_Dim,StateID.P_ID:StateID.P_ID + self.State_Dim] = pred_cov_rl
        self.innovation = self.compute_innovation()
...
class Continuous_PrePosCov_POSCorrectCov_InHiGNSSSatCov(baseEnv):
    # 连续动作的状态预测和协方差预测（该环境只预测位置），输入新息 历史序列 GNSS特征 状态, 输出位置预测
    metadata = {'render.modes': ['human']}
    def __init__(self, config_agent_1, config_agent_2, **kwargs):
        # def __init__(self,trajdata_range, action_scale, discrete_actionspace, reward_setting, trajdata_sort, baseline_mod):
        super(Continuous_PrePosCov_POSCorrectCov_InHiGNSSSatCov, self).__init__(config_agent_1,**kwargs)
        # TODO: 确认 ino和 his 的状态维度
        self.State_Dim = 3
        self.pos_weight = config_agent_1["env_para"]['pos_weight']
        self.observation_space = spaces.Dict(
            {'gnss': spaces.Box(low=-1, high=1, shape=(1, 10)),
             'innovation': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float),
             'History': spaces.Box(low=0, high=1, shape=(1, (self.pos_num-1) * 9), dtype=np.float),
             ...
             'Cov': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float),
             # A2 新增观测
             'POS': spaces.Box(low=0, high=1, shape=(1, 3), dtype=np.float),
             'KF_P': spaces.Box(low=0, high=1, shape=(1, 3), dtype=np.float),
             'innova_cor': spaces.Box(low=0, high=1, shape=(1, self.State_Dim), dtype=np.float),})

        # 设置动作尺度(A1)
        self.pred_scale = config_agent_1["env_para"]['continuous_scale_state_pred']
        self.policy_cov_scale = config_agent_1["env_para"]['continuous_scale_policy_cov']
        # 设置动作尺度(A2)
        self.correct_scale = config_agent_2["env_para"]['continuous_scale_state_pred']
        self.policy_P_scale = config_agent_2["env_para"]['continuous_scale_policy_cov']
    ...
    def reset(self):
        self.innovation_correction = 0.1 * np.ones([1, self.State_Dim])
        obs = super().reset()
        return obs
    ...
    def _normalize_pos(self, pre_state):
        if self.cord == 'NED':
            pre_state[0] = pre_state[0] / 50
            pre_state[1] = pre_state[1] / 50
        elif self.cord == 'LLH':
    ...
    def _next_observation(self):
        ...
        # KF POS feature process
        column = ['Latitude_RLpredict','Longitude_RLpredict','Ellipsoid_Height_RLpredict','Velocity_X_RLpredict','Velocity_Y_RLpredict',
                  'Velocity_Z_RLpredict','Roll_RLpredict','Pitch_RLpredict','Heading_RLpredict']
        cur_state = self.baseline.loc[self.current_step-1, column].values
        if self.cord == 'NED':  # 地理坐标系转到导航坐标系
            n,e,d = pm.geodetic2ned(cur_state[0], cur_state[1], cur_state[2], self.start_pos[0], self.start_pos[1], self.start_pos[2])
            cur_pos = np.array([n, e, d])
        cur_pos = self._normalize_pos(cur_pos)
        ...
        obs_all = {'History': obs.reshape(1, obs.size, order='F'),
                   ...
                   'State': pre_state.reshape(1, pre_state.size),
                   'Cov': rl_cov_diag.reshape(1, rl_cov_diag.size),
                   'POS': cur_pos.reshape(1, cur_pos.size),
                   'KF_P': KF_P.reshape(1, KF_P.size),
                   'innova_cor': self.innovation_correction.reshape(1, self.innovation_correction.size)}

        return obs_all

    def step(self, action):  # modified in 3.3
        # TODO 两个agent动作的顺序需要验证一下
        ...
        # P 调整
        cov_kf_pva = self.giengine.Cov_[StateID.P_ID:StateID.P_ID + self.State_Dim,
                     StateID.P_ID:StateID.P_ID + self.State_Dim].copy()
        d_cov_rl = np.diag(self.policy_P_scale * action[0, 3 * self.State_Dim:].copy())
        # TODO 协方差预测不当会造成融合后对角线变为0
        if self.prdcov_mode == "Add_mode2":
            cov_kf_pva = cov_kf_pva + d_cov_rl
            cov_kf_pva[cov_kf_pva < 1e-1] = 1e-1
        ...
        self.giengine.Cov_[StateID.P_ID:StateID.P_ID + self.State_Dim,
            StateID.P_ID:StateID.P_ID + self.State_Dim] = pred_cov_rl
        self.innovation_correction = self.compute_innovation()
