...
traj_sum_df = pd.read_csv(f'{dir_path}/env/raw_tripID_SZdata_split.csv')
traj_sum_df_full_traj = pd.read_csv(f'{dir_path}/env/raw_tripID_SZdata.csv')

traj_openroad_rtk = traj_sum_df.loc[(traj_sum_df['Type']=='openroad')]['tripID'].values.tolist()
traj_canyon_rtk = traj_sum_df.loc[(traj_sum_df['Type']=='canyon')]['tripID'].values.tolist()
traj_forest_rtk = traj_sum_df.loc[(traj_sum_df['Type']=='forest')]['tripID'].values.tolist()
traj_overpass_rtk = traj_sum_df.loc[(traj_sum_df['Type']=='overpass')]['tripID'].values.tolist()
traj_SZ_RTK = traj_sum_df_full_traj['Traj_ID'].values.tolist()

exclude_traj = ['SZ-20240729-1354-1451/RTK-canyon1-0']

trip_type_maping={
    'SZ_openroad_RTK': traj_openroad_rtk, # 切换轨迹时候会不平稳
    'SZ_openroad_RTK_A2': traj_openroad_rtk, # 切换轨迹时候会不平稳
    'SZ_canyon_RTK': traj_canyon_rtk,
    'SZ_forest_RTK': traj_forest_rtk,
    'SZ_overpass_RTK': traj_overpass_rtk,
    'SZ_RTK': traj_SZ_RTK,
}
