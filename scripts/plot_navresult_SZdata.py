...
def plotNavresult(navresult_filepath,refresult_filepath,config):

    navresult = np.loadtxt(navresult_filepath)

    ref_column = ['GPS TOW (s)', 'UnixTimeMillis_ref', 'latitude(deg)', 'longitude(deg)', 'height(m)']

    if 'GT' in refresult_filepath:
        colums_gnss = ['UnixTimeMillis_ref', 'latitude(deg)', 'longitude(deg)', 'Altitude(m)','Geoid Separation{m}']
        target_cols = [1, 2, 4, 9, 11]
        ref_pd = pd.read_table(refresult_filepath, sep=',', header=None, usecols=target_cols) #
        ref_pd.columns = colums_gnss
        ref_pd[['latitude(deg)', 'longitude(deg)']] = ref_pd[['latitude(deg)', 'longitude(deg)']].apply(
            lambda x: x // 100 + (x % 100) / 60)

        valid_data = (ref_pd['Geoid Separation{m}'].notna()) & (ref_pd['Geoid Separation{m}'] != 0)  # 判断是否有椭球高修正
        if valid_data.any():
            ref_pd['height(m)'] = ref_pd['Altitude(m)'] + ref_pd['Geoid Separation{m}']
        else:
            ref_pd['height(m)'] = ref_pd['Altitude(m)']
            ref_pd['Geoid Separation{m}'] = ref_pd['Geoid Separation{m}'].fillna(0)

        # 时间戳转换
        date = config["date"]
        time_str_series = ref_pd.iloc[:, 0].apply(lambda x: f"{float(x):010.3f}")
        # 第 2 步：把日期和时间无缝拼接起来 (变成类似 "28072024080624")
        combined_str = date + time_str_series
        # 第 3 步：转换为 datetime 对象，并强制指定为 UTC 时区
        dt_obj = pd.to_datetime(combined_str, format='%d%m%Y%H%M%S.%f').dt.tz_localize('UTC')
        # 第 4 步：转换为 Unix 时间戳 (秒)
        # astype('int64') 会变成纳秒，除以 10**9 变成标准的秒级时间戳
        ref_pd['UnixTimeMillis_ref'] = dt_obj.astype('int64') / 10 ** 9
    ...
    for i in range(len(pos_gt)):
        delta_gt = refresult[i, 2:5] - navresult[0, 2:5]
        pos_gt[i, 1:4] = drad2dm(rm, rn, blh_station, delta_gt).reshape(1, 3)

    print('plotting estimated navigation result!')

    # 绘图
    plt.figure('horizontal position')
    plt.plot(pos[:, 2], pos[:, 1], label='ESKF')
    plt.plot(pos_gt[:, 2], pos_gt[:, 1], label='GT')
    # plt.plot(refresult[:, 2], refresult[:, 1], label=['GT'])
    plt.axis('equal')
    plt.legend()
    plt.xlabel('East [m]')
    plt.ylabel('North [m]')
...
def plotNavError(navresult_filepath, refresult_filepath, gnsspath,skiprows,config):
    ...
    naverror = calcNavresultError(navresult_filepath, refresult_filepath, gnsspath,skiprows,config) # , gnsserror
    ...
    plt.legend([f'GNSS/INS (N):{np.mean(np.abs(naverror[:,2])):.2f}m', f'GNSS/INS (E):{np.mean(np.abs(naverror[:,3])):.2f}m',
                f'GNSS/INS (D):{np.mean(np.abs(naverror[:,4])):.2f}m'])
    plt.xlabel('Time [s]')
    plt.ylabel('Error [m]')
...
def calcNavresultError(navresult_filepath, refresult_filepath, gnsspath,skiprows,config):
    ...
    refinter = np.zeros((navresult.shape[0], 5))
    ...
    lats_corrected, lons_corrected, atts_corrected = apply_lever_arm_compensation(refinter[:,2]/D2R,refinter[:,3]/D2R,refinter[:,4],navresult[:,10],GTlever[0],GTlever[1],GTlever[2])
    ...
    # 误差计算
    naverror[:, 2:5] = navresult[:, 2:5] - refinter[:, 2:5]
...
if __name__ == '__main__':
    ...
    """
    SZ-20240728-1520-1641 SZ-20240729-1606-1716 SZ-20240729-1354-1451
    """
    Dataset = 'SZ-20240729-1354-1451'
    ...
    # 导航结果
    plotNavresult(navresult_filepath,refresult_filepath,config)
    # 计算并绘制导航误差
