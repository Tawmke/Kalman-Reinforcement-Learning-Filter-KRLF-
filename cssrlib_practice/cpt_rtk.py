...
# 基本路径设置
bdir = '/mnt/sdb/home/tangjh/KF-GINS-Py-main/data_cpt/'
# navfile = bdir+'SEPT078M.21P'
# obsfile = bdir+'SEPT078M1.21O'
# basefile = bdir+'3034078M1.21O'
navfile = bdir+'brdm0870.19p'
obsfile = bdir+'cpt0870.19o'
basefile = bdir+'cpt0870_base.o'
atxfile = bdir+"igs14.atx"
np.set_printoptions(legacy='1.25')  # 新增, 使得SciView视图不显示数值类型, 只显示数值 Chen251014

# 读取真值文件并转换为GPST时间
ref_colmun = ['gps_week', 'sow', 'pos_x','pos_y','pos_z','vel_x','vel_y','vel_z','att_x','att_y','att_z']
ref_pd_retiming = pd.read_csv(bdir+'reference.csv',names=ref_colmun, header=None)
leap_seconds = 18
gps_to_unix_epoch_seconds = 315964800
ref_pd_retiming['UnixTimeMillis_ref'] = (
 (ref_pd_retiming['gps_week'] * 604800 + ref_pd_retiming['sow'] - leap_seconds + gps_to_unix_epoch_seconds) * 1000
).astype('int64')  # GPS周和周内秒转化为UnixTimeMillis，已验证无误

ref_xyz = np.array([ref_pd_retiming['pos_x'], ref_pd_retiming['pos_y'], ref_pd_retiming['pos_z']])
ref_XYZ2llh = np.array(
        [coord.ecef2geodetic(ref_xyz[:, i]) for i in range(ref_xyz.shape[1])])

ref_pd_retiming['LatitudeDegrees'] = ref_XYZ2llh[:,0]
ref_pd_retiming['LongitudeDegrees'] = ref_XYZ2llh[:,1]
ref_pd_retiming['AltitudeMeters'] = ref_XYZ2llh[:,2]
...
sigs = [rSigRnx("GC1C"),  rSigRnx("GL1C"), rSigRnx("GD1C"), rSigRnx("GS1C"),
        rSigRnx("GC2W"),  rSigRnx("GL2W"), rSigRnx("GD2W"), rSigRnx("GS2W"),
...
sigsb = [rSigRnx("GC1C"),  rSigRnx("GL1C"), rSigRnx("GD1C"), rSigRnx("GS1C"),
        rSigRnx("GC2W"),  rSigRnx("GL2W"), rSigRnx("GD2W"), rSigRnx("GS2W")]
         # rSigRnx("EC1C"), rSigRnx("EL1C"), rSigRnx("ES1C")]
...
nav.rb = [-2408697.451,4698107.975,3566695.2035]  # 综合场景基站真值
...
nep = min(int(nepoch_r/2), nepoch_b)
