...
import scipy.io
sys.path.append('/mnt/sdb/home/tangjh/KF-GINS-Py-main/5.1 RTK_practice/cssrlib')
...
navfile = bdir+'hkqt137c.21n'
obsfile = bdir+'UrbanNav-HK-Medium-Urban-1.ublox.f9p.obs'
basefile = bdir+'hkqt137c.21o'
reffile = bdir+'UrbanNav_TST_GT_raw.txt'
# atxfile = bdir+"igs14.atx"
np.set_printoptions(legacy='1.25')  # 新增, 使得SciView视图不显示数值类型, 只显示数值 Chen251014

# 读取真值文件并转换为GPST时间
ref_colmun = ['UnixTimeMillis_ref','gps_week', 'sow', 'lat(d)','lat(m)','lat(s)','lon(d)','lon(m)','lon(s)','AltitudeMeters',
              'vel_x','vel_y','vel_z','acc_x','acc_y','acc_z','roll','pitch','heading','Q']
ref_pd_retiming = pd.read_csv(reffile, sep='\s+',skiprows=2, header=None, names=ref_colmun)
ref_pd_retiming['LatitudeDegrees'] = ref_pd_retiming['lat(d)'] + ref_pd_retiming['lat(m)']/60 + ref_pd_retiming['lat(s)']/3600
ref_pd_retiming['LongitudeDegrees'] = ref_pd_retiming['lon(d)'] + ref_pd_retiming['lon(m)']/60 + ref_pd_retiming['lon(s)']/3600

ref_llh = np.array([ref_pd_retiming['LatitudeDegrees'], ref_pd_retiming['LongitudeDegrees'], ref_pd_retiming['AltitudeMeters']])
ref_llh2XYZ = np.array(
        [coord.geodetic2ecef(ref_llh[:, i]) for i in range(ref_llh.shape[1])])

ref_pd_retiming['pos_x'] = ref_llh2XYZ[:,0]
ref_pd_retiming['pos_y'] = ref_llh2XYZ[:,1]
ref_pd_retiming['pos_z'] = ref_llh2XYZ[:,2]
...
# 根据obs文件调整观测信号类型
sigs = [rSigRnx("GC1C"),  rSigRnx("GL1C"), rSigRnx("GD1C"), rSigRnx("GS1C"),
        rSigRnx("GC2L"),  rSigRnx("GL2L"), rSigRnx("GD2L"), rSigRnx("GS2L"),
        rSigRnx("RC1C"), rSigRnx("RL1C"), rSigRnx("RD1C"), rSigRnx("RS1C"),
        rSigRnx("RC2C"), rSigRnx("RL2C"), rSigRnx("RD2C"), rSigRnx("RS2C")]
        # rSigRnx("EC1C"), rSigRnx("EL1C"), rSigRnx("ES1C")]
        # rSigRnx("CC1I"), rSigRnx("CC7I"), rSigRnx("CS1I")]

sigsb = [rSigRnx("GC1C"),  rSigRnx("GL1C"), rSigRnx("GS1C"),
        rSigRnx("GC2W"),  rSigRnx("GL2W"), rSigRnx("GS2W"),
        rSigRnx("RC1C"), rSigRnx("RL1C"), rSigRnx("RS1C"),
        rSigRnx("RC2C"), rSigRnx("RL2C"), rSigRnx("RS2C")]
         # rSigRnx("EC1C"), rSigRnx("EL1C"), rSigRnx("ES1C")]
...
dec = rn.rnxdec()
dec.setSignals(sigs)
nav = gn.Nav(nf=2)  # nav.pmode默认为1，即默认为kinematic; nav.nf默认为2，即默认跟踪每颗卫星的2个频率（双频接收机）
...
nav.rb = [-2421567.8916, 5384910.5631, 2404264.3943]  # 综合场景基站真值
...
dec.pos = [-2421567.8916, 5384910.5631, 2404264.3943]
...
for ne in range(nep):
    obs, obsb = rn.sync_obs(dec, decb, dt_th=1)  # 基准站与用户观测频率不一致，需提高时间戳匹配精度 Chen251024
    if ne == 0:
        t0 = nav.t = obs.t
    rtk.process(ne,obs, obsb=obsb)
...
true_XYZ = np.array([baseline['pos_x'], baseline['pos_y'], baseline['pos_z']])
...
ax.plot3D(rtk_XYZ[0,:], rtk_XYZ[1,:], rtk_XYZ[2,:],
