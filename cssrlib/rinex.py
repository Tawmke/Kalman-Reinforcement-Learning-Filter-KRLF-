...
class rnxdec:
    ...
    def autoSubstituteSignals(self):
        ...
        substitutions = []  # 记录替换对  新增Chen251021
        ...
        for sys, tmp in self.sig_tab.items():
            for typ, sigs in tmp.items():
                for i, sig in enumerate(sigs):
                    ...
                    # 注释以下3行，改用后面的几行，以打印替换信息 Chen251021
                    # for a in atts:
                    #     if sig.toAtt(a) in self.sig_map[sys].values():
                    #         self.sig_tab[sys][typ][i] = sig.toAtt(a)

                    # 尝试替换
                    original_sig = str(sig)
                    for a in atts:
                        candidate_sig = sig.toAtt(a)
                        if candidate_sig in self.sig_map[sys].values():
                            self.sig_tab[sys][typ][i] = candidate_sig
                            substitutions.append((original_sig, str(candidate_sig)))
                            break

            # 打印替换结果  新增Chen251021
            if substitutions:
                print(f"完成 {len(substitutions)} 个信号替换:")
    ...
    def decode_obs(self):
        ...
        for line in self.fobs:
            ...
            obs.lli = np.empty((0, self.nsig[uTYP.L]), dtype=np.int32)  # 失锁指示器
            ...
            for _ in range(nsat):
                ...
                for i, sig in self.sig_map[sys].items():
                    ...
                    #
                    sval = line[16 * i + 3:16 * i + 17].strip()  # 观测值字符串
                    slli = line[16 * i + 17] if len(line) > 16 * i + 17 else ''  # 失锁指示器
    ...
    # 新增函数 Chen251031
    def get_obs_timestamps(self, filename):
        """
        读取RINEX .obs文件中的所有历元时间戳
        返回：
        ...
        with open(filename, 'r') as f:
            for line in f:
                if line.startswith('>'):
                    parts = line[1:32].split()
                    if len(parts) >= 6:
                        y, m, d, h, mi, s = map(float, parts[:6])
                        timestamps.append(datetime.datetime(int(y), int(m), int(d), int(h), int(mi), int(s)))
        n = len(timestamps)
        return timestamps, n
...
def sync_obs(dec, decb, dt_th=0.1):
    ...
    obs = dec.decode_obs()
    obsb = decb.decode_obs()  # decode_obs遵循迭代器模式, 每次调用返回下一个时间点的完整观测数据集

    # import cssrlib.gnss as gn
    # obs_gpst = gn.time2str(obs.t)
    # obsb_gpst = gn.time2str(obsb.t)
    ...
    # obs_gpst_sync = gn.time2str(obs.t)
    # obsb_gpst_sync = gn.time2str(obsb.t)
