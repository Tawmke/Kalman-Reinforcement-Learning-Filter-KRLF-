    ...
    while j >= 0:
        if j <= k:
            for i in range(j+1, n):
                mu = np.round(L[i, j])  # 取整
                if mu != 0.0:
                    L[i:, j] -= mu*L[i:, i]
                    Z[:, j] -= mu*Z[:, i]

        delta = d[j]+L[j+1, j]**2*d[j+1]
        if delta+1e-6 < d[j+1]:  # permutation  排序
            eta = d[j]/delta
...
def sr_boost(d):
    ...
    Ps = np.prod(2*norm.cdf(0.5/np.sqrt(d))-1)  # np.prod: 计算数组中所有元素的乘积; 2*norm.cdf(0.5/np.sqrt(d))-1: 禁地看成在当前模糊度方差下固定成功的概率（提供概率下界值）
    return Ps
...
def msearch(L, d, ahat, ncands=2):
    ...
    dist = np.zeros(n)
    acond = np.zeros(n)  # 浮点解条件估值
    zcond = np.zeros(n, dtype=np.int32)  # 整数解条件估值
...
def mlambda(ahat, Qahat, ncands=2, armode=1, P0=0.995):
    ...
    L, d, Z = reduction(L, d)  # 整数高斯变换与条件置换实现降相关
    iZt = np.round(inv(Z.T))  # Z.T的逆（由于浮点计算，inv(Z.T)可能不是精确的整数矩阵，所以使用np.round取整）
    zhat = Z.T@ahat  # Z变换
