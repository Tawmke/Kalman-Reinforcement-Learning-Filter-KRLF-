...
class pppos():
    ...
    def __init__(self, nav, pos0=np.zeros(3),
        ...
        self.nav.phw_opt = phw_opt  # 相位缠绕处理方式
        ...
        self.nav.na = (3 if self.nav.pmode == 0 else 6)  # 基础状态变量【位置(+速度+对流层+电离层)】维度
        self.nav.nq = (3 if self.nav.pmode == 0 else 6)  # 基础状态变量【位置(+速度+对流层+电离层)】过程噪声维度

        self.nav.na += self.nav.ntrop + self.nav.niono   # +对流层+电离层
        self.nav.nq += self.nav.ntrop + self.nav.niono

        # State vector dimensions (including slant iono delay and ambiguities)
        #
        self.nav.nx = self.nav.na+uGNSS.MAXSAT*self.nav.nf  # 总状态维度 = 基础 + 大气 + 模糊度
        ...
        #
        self.nav.sig_p0 = 30.0   # [m]         原: 100 Chen251028
        self.nav.sig_v0 = 1.0     # [m/s]     原: 1.0 Chen251028
        self.nav.sig_ztd0 = 0.1  # [m]
        self.nav.sig_ion0 = 10.0  # [m]
        self.nav.sig_n0 = 30.0
        ...
        self.nav.parmode = 2  # 1: normal, 2: PAR（部分模糊度固定策略）
        ...
        dP.flags['WRITEABLE'] = True  # 强制启用NumPy数组的写入权限, 确保可以对数组进行修改
        ...
        # ambiguity  self.nav.q只存储基础状态向量, 因此这里的赋值操作实际无用
        if self.nav.pmode >= 1:  # kinematic
    ...
    def varerr(self, nav, el, f):
        ...
        s_el = max(np.sin(el), 0.1*rCST.D2R)  # 确保最小值不低于0.1度
    ...
    def udstate(self, obs):
        ...
        tt = timediff(obs.t, self.nav.t)  # self.nav.t存储的是上一解算历元时间（第一个历元等于当前历元）
        ...
        for f in range(self.nav.nf):
            ...
            # Reset phase-ambiguity if instantaneous AR  连续记录历元数超过最大值 maxout=5
            # or expire obs outage counter  观测数据被编辑标记 (edt[i, :] > 0)
            #
            for i in range(uGNSS.MAXSAT):
                ...
                self.nav.outc[i, f] += 1  # 统计卫星连续不可见的历元数, 一开始假设卫星不可见（置1）, 后面会有逻辑把确实可见的重新置0（第1460行）
                reset = (self.nav.outc[i, f] >
                         self.nav.maxout or np.any(self.nav.edt[i, :] > 0))  # 连续 5 个历元不可见或者卫星质量有问题, 就重置
                if sys_i not in obs.sig.keys():
                    continue

                # Reset ambiguity estimate
                #
                j = self.IB(sat_, f, self.nav.na)  # 获取模糊度状态索引
                ...
                if self.nav.niono > 0:  # 如果估计电离层
                    # Reset slant ionospheric delay estimate
                    #
                    j = self.II(sat_, self.nav.na)  # 获取电离层状态索引
                    if reset and self.nav.x[j] != 0.0:
                        self.initx(0.0, 0.0, j)
            ...
            for i in range(ns):
                ...
                if self.nav.nf > 1 and self.nav.niono > 0:
                    ...
                    #
                    sig1 = obs.sig[sys[i]][uTYP.C][0]  # 总是用频率0和1
                    ...
                    # Get iono delay at frequency of first signal  第一个频率的电离层延迟
                    #
                ...
                cp = obs.L[i, f]  # RTK: 站间单差载波相位; PPP/PPP-RTK: 非差载波相位
                pr = obs.P[i, f]
                if cp == 0.0 or pr == 0.0 or lam is None:
                    continue

                bias[i] = cp - pr/lam + 2.0*ion[i]/lam*(f1/fi)**2  # 利用伪距粗略估计整周模糊度（RTK模式下获得的是单差整周模糊度）
            ...
            for i in range(ns):
                ...
                j = self.IB(sat[i], f, self.nav.na)  # 获取模糊度状态索引
                if bias[i] != 0.0 and self.nav.x[j] == 0.0:

                    self.initx(bias[i], self.nav.sig_n0**2, j)  # 对self.nav.x中的整周模糊度及其P矩阵对应位置的元素进行赋值
    ...
    def zdres(self, obs, cs, bsx, rs, vs, dts, rr, rtype=1):
        ...
        for i in range(n):
            ...
            cbias = np.zeros(self.nav.nf)  # 相位偏差
            pbias = np.zeros(self.nav.nf)  # 码偏差
            ...
            # Range correction
            #
            prc[i, :] = trop + antrPR + antsPR + iono - cbias    # [对流层延迟, 接收机天线相位中心改正, 卫星天线相位中心改正, 电离层延迟, 码硬件延迟偏差]
            cpc[i, :] = trop + antrCP + antsCP - iono - pbias + phw  # [..., 载波相位硬件延迟偏差, 相位缠绕]

            r += relatv - _c*dts[i]  # 计算经过相对论效应和卫星钟差校正后的几何距离

            for f in range(nf):
                y[i, f] = obs.L[i, f]*lam[f]-(r+cpc[i, f])  # 载波相位残差[测量值（转换为米）- 校正过的几何距离 - 各误差项]
                y[i, f+nf] = obs.P[i, f]-(r+prc[i, f])      # 伪距残差[测量值 - 校正过的几何距离 - 各误差项]
    ...
    def sdres(self, obs, x, y, e, sat, el):
        ...
        mode = 1 if len(y) == ns else 0  # 0:DD,1:SD  y可能存有基站观测值
        ...
        H = np.zeros((ns*nf*2, self.nav.nx))  # shape: [星数*频率数*(载波+伪距), 状态向量维度]
        v = np.zeros(ns*nf*2)
        ...
        for sys in obs.sig.keys():
            ...
            for f in range(0, nf*2):
                ...
                for j in idx:
                    ...
                    if mode == 0:  # DD
                        v[nv] = (y[i, f]-y[i+ns, f])-(y[j, f]-y[j+ns, f])  # 先站间作差, 再星间作差
                    else:
                        #  Single-difference measurement
                        #
                        v[nv] = y[i, f] - y[j, f]  # 星间作差

                    # SD line-of-sight vectors
                    #
                    H[nv, 0:3] = -e[i, :] + e[j, :]  # 基站位置已知, 偏导数为0, 所以不再做站间差分
                    ...
                    if f < nf:  # carrier-phase
                        ...
                        H[nv, idx_i] = +lami
                        H[nv, idx_j] = -lamj  # 本行及上行合起来即为星间差分
                        v[nv] -= lami*(x[idx_i] - x[idx_j])  # 载波相位还要减去相应的整周模糊度项
                        ...
                        self.nav.vsat[sat[i]-1, f] = 1  # 标记处理过的卫星
                    ...
                    nb[b] += 1  # counter for single-differences per signal  每个系统每个频率下处理的信号数
    ...
    def resamb_lambda(self, sat, armode=1, P0=0.995):
        ...
        nx = self.nav.nx  # 总状态个数, 包含不可见星的整周模糊度
        na = self.nav.na  # 除模糊度外的状态个数
        xa = np.zeros(na)
        ix = self.ddidx(self.nav, sat)  # 获取星间差分关系索引
        nb = len(ix)
        if nb <= 0:
            print("no valid DD")
            return -1, -1
        ...
        y = self.nav.x[ix[:, 0]]-self.nav.x[ix[:, 1]]  # RTK模式下self.nav.x中的整周模糊度是站间单差整周模糊度, 因此这里加上星间差分将形成双差整周模糊度
        DP = self.nav.P[ix[:, 0], na:nx]-self.nav.P[ix[:, 1], na:nx]  # 提取单差模糊度之间的协方差关系
        Qb = DP[:, ix[:, 0]-na]-DP[:, ix[:, 1]-na]  # 构建双差模糊度的完整协方差矩阵
        Qab = self.nav.P[0:na, ix[:, 0]]-self.nav.P[0:na, ix[:, 1]]  # 构建非模糊度参数与双差模糊度的互协方差矩阵
        ...
        if nfix > 0 and (armode == 2 or s[0] <= 0.0 or
            ...
            bias = b[:, 0]                      # 取最优的整数解
            y -= b[:, 0]                        # 计算固定解与浮点解之间的差值
            K = Qab@np.linalg.inv(Qb)           # 计算增益矩阵K
            self.nav.xa -= K@y                  # 使用固定的模糊度修正非模糊度状态参数（如位置等）
            self.nav.Pa -= K@Qab.T              # 更新修正后状态参数的协方差矩阵

            # restore SD ambiguity
            xa = self.restamb(bias, nb)         # 恢复单差模糊度
    ...
    def process(self, ne, obs, cs=None, orb=None, bsx=None, obsb=None):
        """
        ...
        #
        rs, vs, dts, svh, nsat = satposs(obs, self.nav, cs=cs, orb=orb) # 卫星位置速度

        self.nav.nsat[1] = nsat

        if nsat < 6:  # 要求至少6星才开始定位
            print(" too few satellites < 6: nsat={:d},{:d}".format(nsat,ne))
            return
        ...
        if ns < 6: # Tang 251202: 修改为小于5返回，原为6
        ...
        # and iono
        #
        self.udstate(obs_)  # 注意如果是RTK则输入的是站间单差观测量, 如果是PPP/PPP-RTK则输入的是非差观测量
        ...
        #
        yu, eu, elu = self.zdres(obs, cs, bsx, rs, vs, dts, xp[0:3])  # cs: 状态空间表示校正数据; bsx：偏差SINEX文件数据

        # Select satellites having passed quality control
        #
        # index of valid sats in obs.sat
        sat = obs.sat[iu]
        y[:ns, :] = yu[iu, :]  # 非差残差, 此时y还包含有基站观测数据
        e[:ns, :] = eu[iu, :]
        el = elu[iu]
        ...
        #
        ny = y.shape[0]
        if ny < 6:  # 第一次的y可能还包含基站观测数据, 后面第1442行执行y = yu[iu, :]之后才只包含流动站观测数据
            self.nav.P[np.diag_indices(3)] = 1.0
            self.nav.smode = 5  # 浮点解
            return -1
        ...
        if self.valpos(v, R):
            ...
            for i in range(ns):
                j = sat[i]-1
                for f in range(self.nav.nf):
                    if self.nav.vsat[j, f] == 0:  # 差分过程用到的卫星会被标记为1
                        continue
                    self.nav.outc[j, f] = 0
                    if f == 0:
                        self.nav.ns += 1
