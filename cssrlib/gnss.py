            ...
            if sys == uGNSS.GPS:
                ...
                if (s[1] == '1' and s[2] not in 'CESLXZB') or \
                   ...
                   (s[1] == '6' and s[2] not in 'SLXEZ'):
                    raise ValueError
            elif sys == uGNSS.BDS:  # 'DPX' -> 'DPXQI'  Chen251016
                if (s[1] == '2' and s[2] not in 'IQX') or \
                   (s[1] == '1' and s[2] not in 'DPXSLZI') or \
                   (s[1] == '5' and s[2] not in 'DPXQI') or \
...
class Nav():
    ...
    def __init__(self, nf=2):
        ...
        self.eop = np.zeros(9)
        self.elmin = np.deg2rad(15.0)  # 截止高度角
        self.tidecorr = False
        self.nf = nf
        self.ne = 0
        self.nc = 0
