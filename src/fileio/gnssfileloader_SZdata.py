...
下方提供了rtklib生成的pos和nmea格式处理，只用于深圳数据集
创建人：唐健浩
"""
leap_seconds = 18
...
class GnssFileLoader:
    def __init__(self, filename: str, skiprows: int, config=None, save_gnss=False):
        ...
        if 'pos' in filename:
            ...
            #     # if 'BESTPOSA' in line and 'FINE' in line: # 找到当前日期
            #     #     split_line = [item.strip() for item in line.strip().split(',')]
            #     #     gps_week = int(split_line[5])
            #     #     break
            date = config["date"]

            for idx, line in enumerate(filelines):
                if 'GGA' in line and 'E' in line:
                    ...
                    for nextline in reversed(filelines[:idx]): # 从下往上遍历
                        ...
                        if 'BESTPOSA' in nextline: # std 数据
                            split_line = [item.strip() for item in nextline.strip().split(',')]
                            split_line[1] = UnixTime
            ...
            # 用计算出的std代替算出DOP
            sigma_base = 0.025 # 底噪
            gnss_dop_pd = pd.DataFrame()
            gnss_dop_pd['UnixTimeMillis_ref'] = std_pd['UnixTimeMillis_ref']
            sdn = std_pd['sdn(m)']
            sde = std_pd['sde(m)']
            sdu = std_pd['sdu(m)']
            gnss_dop_pd['PDOP'] = np.sqrt(sdn ** 2 + sde ** 2 + sdu ** 2) / sigma_base
            gnss_dop_pd['HDOP'] = np.sqrt(sdn ** 2 + sde ** 2) / sigma_base
            gnss_dop_pd['VDOP'] = sdu / sigma_base
            ...
            if save_gnss:
                ...
                rtk_column = ['UnixTimeMillis_ref', 'latitude(deg)', 'longitude(deg)', 'height(m)','sdn(m)', 'sde(m)','sdu(m)','Satnum',
                              'CNR_Mean','CNR_Q75','CNR_Q25','EA_Mean','EA_Q75','EA_Q25','PDOP', 'HDOP','Q']
            rtk_pd.dropna(inplace=True)
            rtk_pd.drop_duplicates(subset=['UnixTimeMillis_ref'], keep='first', inplace=True)
            self.data_ = rtk_pd[rtk_column].values.astype(np.float64)
