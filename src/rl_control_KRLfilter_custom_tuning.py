...
from env.Urbannav_GNSSINS_KRLF import * # RL环境
...
def main_process(config):
    ...
    if True:
        ...
        if envmod == 'InHiGNSSSta_PreCovSta':
            env = DummyVecEnv([lambda: Continuous_PreStaCov_InHiGNSSStaCov(config, **kwargs)])
            ...
            env = DummyVecEnv([lambda: Continuous_PrePosAttCov_InHiGNSSStaCov(config, **kwargs)])
            encoder = CustomATF1_GnssHisInnStateCov
        elif envmod == 'InHiGNSSPos_CorrectCovPosAtt':
            env = DummyVecEnv([lambda: Continuous_CorrectPosAttCov_InHiGNSSPosCov(config, **kwargs)])
            encoder = CustomATF1_GnssHisInnStateCov
        elif envmod == 'InHiGNSSPos_CorrectCovPos':
            env = DummyVecEnv([lambda: Continuous_CorrectPosCov_InHiGNSSPosCov(config, **kwargs)])
            encoder = CustomATF1_GnssHisInnStateCov
        ...
        tensorboard_log = f'{dir_path}records_values/{running_date}/{triptype}_{traj_type_target_train[1]}_{ratio}_{reward_setting}_{envmod}/' \
                          f'lr={learning_rate:.4f}_pos={posnum}_SP={scale_state_pred:.2f}_' \
                          f'AP={scale_att_pred:.4f}_AW={att_weight:.2f}'
        ...
        for testtype in moreteststypelist:
            ...
            for test_traj in test_trajlist:
                ...
                if envmod == 'InHiGNSSSta_PreCovSta':
                    ...
                    env = DummyVecEnv([lambda: Continuous_CorrectPosAttCov_InHiGNSSPosCov(config, **test_kwargs)])

                elif envmod == 'InHiGNSSPos_CorrectCovPos':
                    env = DummyVecEnv([lambda: Continuous_CorrectPosCov_InHiGNSSPosCov(config, **test_kwargs)])
...
def objective(trial, mode_list, config_tuning_base):
    ...
    ## 噪声参数
    if config_tuning["training_settings"]["triptype"] == 'HK_De_ublox':
        if 'env_param' in mode_list:
            # continuous_scale_state_pred = trial.suggest_float("continuous_scale_state_pred", 1,2.5, log=True)
            # config_tuning["env_para"]["continuous_scale_state_pred"] = continuous_scale_state_pred
            continuous_scale_policy_cov = trial.suggest_float("continuous_scale_policy_cov", 1.0e-4,1.0e-2, log=True)
            config_tuning["env_para"]["continuous_scale_policy_cov"] = continuous_scale_policy_cov
            # initial_RLcov = trial.suggest_float("initial_RLcov", 9, 16, log=False)
        ...
        if 'vel_att_param' in mode_list:
            # continuous_scale_vel_pred = trial.suggest_float("continuous_scale_vel_pred", 1.0e-4,5.0e-2, log=True) # 1.0e-4,5.0e-2
            # config_tuning["env_para"]["continuous_scale_vel_pred"] = continuous_scale_vel_pred
            ...
            # vel_weight = trial.suggest_float("vel_weight", 0,1, log=False) # 0,1
            # config_tuning["env_para"]["vel_weight"] = vel_weight
            att_weight = trial.suggest_float("att_weight", 0,1, log=False) # 0,1
            config_tuning["env_para"]["att_weight"] = att_weight

    elif config_tuning["training_settings"]["triptype"] == 'HK_Me_ublox':
        if 'env_param' in mode_list:
        ...
        if 'model_param' in mode_list:
            postraj_num = trial.suggest_int("postraj_num", 5, 8, log=False)
            config_tuning["env_para"]["postraj_num"] = postraj_num
            ent_coef = trial.suggest_float("ent_coef", 0, 0.05, log=False)
            config_tuning["model_para"]["ent_coef"] = ent_coef
            vf_coef = trial.suggest_float("vf_coef", 0.4, 0.6, log=False)
            config_tuning["model_para"]["vf_coef"] = vf_coef
            clip_range = trial.suggest_float("clip_range", 0.15, 0.25, log=False)
            config_tuning["model_para"]["clip_range"] = clip_range
    ...
    elif config_tuning["training_settings"]["triptype"] == 'HK_Ha_phone':
        if 'env_param' in mode_list:
            continuous_scale_state_pred = trial.suggest_float("continuous_scale_state_pred", 2,3.2, log=True)
            config_tuning["env_para"]["continuous_scale_state_pred"] = continuous_scale_state_pred
            continuous_scale_policy_cov = trial.suggest_float("continuous_scale_policy_cov", 1.0e-3,1.0e-2, log=True)
            config_tuning["env_para"]["continuous_scale_policy_cov"] = continuous_scale_policy_cov
            initial_RLcov = trial.suggest_float("initial_RLcov", 8, 30, log=False)
            config_tuning["env_para"]["initial_RLcov"] = initial_RLcov
            # RL_reset_step = trial.suggest_int("RL_reset_step", 1, 3, log=False)
            # config_tuning["env_para"]["RL_reset_step"] = RL_reset_step

        if 'model_param' in mode_list:
            clip_range = trial.suggest_float("clip_range", 0.05, 0.3, log=False)
            config_tuning["model_para"]["clip_range"] = clip_range
        ...
        if 'second_param' in mode_list:
            buffer_options = [32,64,128]
            n_steps = trial.suggest_categorical("reply_buffer_size", buffer_options)
            n_steps = trial.suggest_categorical("reply_buffer_size", buffer_options)
            config_tuning["model_para"]["n_steps"] = n_steps
            postraj_num = trial.suggest_int("postraj_num", 5, 8, log=False)
    ...
    elif config_tuning["training_settings"]["triptype"] == 'HK_De_ublox_A2':
        if 'env_param' in mode_list:
            continuous_scale_state_pred = trial.suggest_float("continuous_scale_state_pred", 0.02,0.4, log=True)
            config_tuning["env_para"]["continuous_scale_state_pred"] = continuous_scale_state_pred
            continuous_scale_policy_cov = trial.suggest_float("c", 1.0e-4,5.0e-2, log=True)
            config_tuning["env_para"]["continuous_scale_policy_cov"] = continuous_scale_policy_cov
        ...
        if 'vel_att_param' in mode_list:
            continuous_scale_att_pred = trial.suggest_float("continuous_scale_att_pred", 1.0e-3,8.0e-1, log=True) # 1.0e-3,5.0e-1
            config_tuning["env_para"]["continuous_scale_att_pred"] = continuous_scale_att_pred
            att_weight = trial.suggest_float("att_weight", 0,2, log=False) # 0,1
            config_tuning["env_para"]["att_weight"] = att_weight

    elif config_tuning["training_settings"]["triptype"] == 'HK_Me_ublox_A2':
        if 'env_param' in mode_list:
            continuous_scale_state_pred = trial.suggest_float("continuous_scale_state_pred", 0.08,1.2, log=True)
            config_tuning["env_para"]["continuous_scale_state_pred"] = continuous_scale_state_pred
...
def tuning(mode_list,config_tuning):
    ...
    storage_name = f"sqlite:///my_tuning_{running_date}_{triptype}.db"
    ...
    study.optimize(lambda trial: objective(trial, mode_list, config_tuning), n_trials=50,gc_after_trial=True,show_progress_bar=True, n_jobs=2)
...
if __name__ == "__main__":
    traj_list = ['HK_Me_ublox_A2'] # , 'HK_Me_ublox', 'HK_De_ublox', 'TK_Od', 'HK_Ha_ublox' HK_Ha_phone HK_De_ublox_A2 HK_Me_ublox_A2
    ...
    for idx in range(len(gpu_list)):
        for triptype in traj_list:
            ...
            if 'Att' in envmode:
                mode_list_1 = ['env_param','model_training','vel_att_param'] # 'env_param','model_param','model_training','vel_att_param'
            else:
                mode_list_1 = ['env_param', 'model_training']
            process_1 = multiprocessing.Process(target=tuning, args=(mode_list_1,config,))
