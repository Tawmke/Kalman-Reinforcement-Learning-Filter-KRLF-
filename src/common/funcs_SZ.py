...
def lord_GT(refresult_filepath,navresult,gnssresult,config):
    ...
    lats_corrected, lons_corrected, atts_corrected = apply_lever_arm_compensation(refinter[:,2],refinter[:,3],refinter[:,4],navresult[:,10],GTlever[0],GTlever[1],GTlever[2])
    ...
    naverror[:, 2:5] = navresult[:, 2:5] - refinter_re[:, 2:5]
    ...
    return navresult, refinter, nav_pos_err
...
def calcNavresultError(navresult_filepath, refresult_filepath, config):
    ...
    GTlever = config["GTlever"]
    refinter_or = refinter.copy()
    lats_corrected, lons_corrected, atts_corrected = apply_lever_arm_compensation(refinter[:, 2], refinter[:, 3],refinter[:, 4], navresult[:, 10],
                                                                                  GTlever[0], GTlever[1], GTlever[2])
    refinter[:, 2], refinter[:, 3], refinter[:, 4] = lats_corrected, lons_corrected, atts_corrected
