...
class RotationUtils:
    ...
    def euler2matrix(euler):
        rotation = Rotation.from_euler('ZYX', euler, degrees=False) # 从欧拉角创建旋转对象
        ma = rotation.as_matrix() # 旋转矩阵
        ma = np.flip(ma, axis=(0,1)) # 反转数组的指定维度
        return ma.T
