...
"""
可用香港UrbanNav数据集实现RTK/INS组合导航，该代码遍历所有轨迹读取对应参数表yaml，生成打包数据
创建人：唐健浩
"""

starting_flag = True # 设置为 False，则从某个特定traj开始遍历
load_nav_data = True # 设置为 True，可以直接导入已有导航数据
starting_traj = '1_UrbanNav-HK-Medium-Urban-1/ublox_f9p_splitter'
exclude_traj = ['1_UrbanNav-HK-Medium-Urban-1/ublox_m8t_GC','2_UrbanNav-HK-Deep-Urban-1/ublox_m8t_GC',
                '3_UrbanNav-HK-Harsh-Urban-1/ublox_m8t_GC']

if __name__ == "__main__":
    savepath = Path(src_dir+'/dataset_Urbannav/')
    gnss_data_dic = {}
    data_truth_dic = {}
    imu_data_dic = {}
    trip_result = []
    for i, dirname in enumerate(tqdm(sorted(gl.glob(f'{src_dir}/dataset_Urbannav/*/*/')))):
        ...
        device_type = device_mapping.get(equip, "unknown")
        ...
        navresult, refinter, gnssinter, nav_pos_err,nav_vel_err,nav_att_err,gnss_pos_err\
            = lord_GT(refresult_filepath, raw_result_nav, gnssfile.data_, config) # 时间戳对齐，得到真值

        print(f'Nav: pos RMSE: {nav_pos_err}, vel RMSE: {nav_vel_err}, att RMSE: {nav_att_err}')
        print(f'gnss: pos RMSE: {gnss_pos_err}')

        pd_data = np.hstack((navresult[:,1:], refinter[:,2:], gnssinter[:,1:4]))
        column_name = ['UnixTimeMillis_ref', ' Latitude (deg)', ' Longitude (deg)', ' Ellipsoid Height (m)',
                  ...
                  ' Roll_GT (deg)', ' Pitch_GT (deg)', ' Heading_GT (deg)', ' Latitude_GNSS (deg)', ' Longitude_GNSS (deg)', ' Ellipsoid Height_GNSS (m)']
        ...
        traj_stats = {
            ...
            'nav_vel_RMSE': nav_vel_err,
            'nav_att_RMSE': nav_att_err,
            'gnss_pos_RMSE': gnss_pos_err,
    ...
    # s11: 保存数据
    with open(src_dir + 'env/raw_baseline_gnssins_Urbannav.pkl', 'wb') as value_file:
        pickle.dump(data_truth_dic, value_file, True)
    value_file.close()
    with open(src_dir + 'env/raw_gnss_data_Urbannav.pkl', 'wb') as value_file:
        pickle.dump(gnss_data_dic, value_file, True)
    value_file.close()
    with open(src_dir + 'env/raw_imu_data_Urbannav.pkl', 'wb') as value_file:
        pickle.dump(imu_data_dic, value_file, True)
    value_file.close()

    df_summary.to_csv(src_dir + 'env/raw_tripID_Urbannav.csv', index=True)
