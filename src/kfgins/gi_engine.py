...
class GIEngine:
    ...
    def gnssUpdate(self, gnssdata:GNSS):
        ...
        antenna_pos = self.pvacur_.pos + Dr_inv @ self. pvacur_.att.cbn @ self.options_.antlever # 导航状态补偿杆臂误差
        
        # GNSS位置测量新息
        # compute GNSS position innovation
        dz = Dr @ (antenna_pos - gnssdata.blh) # 相当于单位变成米，变为在n系下
        ...
        H_gnsspos[0:3,StateID.PHI_ID:StateID.PHI_ID+3] = RU.skewSymmetric(self.pvacur_.att.cbn @ self.options_.antlever) # StateID.PHI 姿态ID
