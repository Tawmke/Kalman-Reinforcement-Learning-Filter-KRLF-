...
def run(src_dir, config):
    ...
    gnssfile = GnssFileLoader(gnsspath, skiprows)
    ...
    # 导航结果
    naverror = calcNavresultError(navresult_filepath, refresult_filepath,config)
    dis_err = np.mean(np.sqrt(naverror[:, 2]**2 + naverror[:, 3]**2 + naverror[:, 4]**2))
    return dis_err
...
def objective(trial):
    ...
    # vrw = trial.suggest_float("vrw", 0.001, 10, log=True)
    ...
    # 杆臂
    antlever_x = trial.suggest_float("antlever_x", 0.5, 5)
    antlever_y = trial.suggest_float("antlever_y", -2, 2)
    antlever_z = trial.suggest_float("antlever_z", 0.5, 3)
...
if __name__ == "__main__":
    ...
    """
    Dataset = '2_UrbanNav-HK-Deep-Urban-1'
    Equip = 'google_pixel4'
    ...
    study.optimize(objective, n_trials=50)  # 跑N组实验
    print("最优参数: ", study.best_params)
    df = study.trials_dataframe()
    # 保存为 CSV
    df.to_csv(config['outputpath'] + "/optimization_results_2.csv", index=False, encoding='utf-8-sig')
