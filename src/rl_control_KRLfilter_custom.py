...
if __name__ == "__main__":
    # 可选环境：'HK_Me_ublox', 'HK_De_ublox', 'TK_Sh' TK_Od HK_Ha_ublox HK_De_ublox_A2 HK_Me_ublox_A2
    triptype = 'HK_Me_ublox_A2'
    runtimes = 5
    ...
    # 是否重复实验
    if config['training_settings']['repeat_train']:
        filename = config['training_settings']['repeat_train'] + '/train_config.yaml'
        with open(filename, 'r', encoding='utf-8') as f:
            config = yaml.load(f, Loader=yaml.FullLoader)  # 从YAML文件(filename)中加载配置数据
    ...
    if config["testing_settings"]['onlytesting'] == False:
        ...
        for _ in range(runtimes):
            if envmod == 'InHiGNSSSta_PreCovSta':
                ...
                env = DummyVecEnv([lambda: Continuous_PrePosCov_InHiGNSSPosCov_RTK(config, **kwargs)])
                encoder = CustomATF1_GnssHisInnStateCov
            # Agent 2 环境
            ...
            tensorboard_log = f'{dir_path}records_values/{running_date}/{triptype}_{traj_type_target_train[1]}_{ratio}_{reward_setting}_{envmod}/' \
                              f'lr={learning_rate:.4f}_pos={posnum}_SP={scale_state_pred:.2f}_PC={scale_policy_cov:.4f}_' \
        ...
        for model_folder in model_folderlist:
            if model_folder == 'lr=0.0002_pos=6_SP=0.33_PC=0.0557_AP=0.1000_AW=0.20':
                start_testing = True
            ...
            for model_sepfolder in model_sepfolderlist:
                ...
                # MA_ublox的bug，config有几个没对上，作出修改
                continuous_scale_state_pred = float(model_folder.split('SP=')[1].split('_PC')[0])
                continuous_scale_policy_cov = float(model_folder.split('PC=')[1].split('_AP')[0])
                learning_rate = float(model_folder.split('lr=')[1].split('_pos=')[0])
                config["env_para"]["continuous_scale_state_pred"] = continuous_scale_state_pred
                config["env_para"]["continuous_scale_policy_cov"] = continuous_scale_policy_cov
                config["training_settings"]["learning_rate"] = learning_rate
                ...
                if process_trig:
                    ...
                    for testtype in moreteststypelist:
                        ...
                        # 保存参数表
                        logdirname = model_loggerdir +'/train_'
                        with open(f'{logdirname}config.yaml', 'w', encoding='utf-8') as f:
                            yaml.dump(config, f, allow_unicode=True, sort_keys=False)
            ...
            for agent_1_sepfolder in agent_1_sepfolderlist:
                for agent_2_sepfolder in agent_2_sepfolderlist:
                    ...
                    if agent_1_process_trig and agent_2_process_trig:
                        ...
                        env = DummyVecEnv([lambda: Continuous_PrePosAttCov_InHiGNSSPosCov(config_agent_1, **kwargs)])
                        obs = env.reset()
                        ...
                        env = DummyVecEnv([lambda: Continuous_CorrectPosAttCov_InHiGNSSPosCov(config_agent_2, **kwargs)])
                        obs = env.reset()
                        encoder = CustomATF1_GnssHisInnPosAttP_A2
                        policy_dim = 0
                        for key, value in obs.items():
                            policy_dim += obs[key].shape[-1]
                        policy_kwargs = dict(features_extractor_class=encoder,features_extractor_kwargs=dict(features_dim=policy_dim),
                        ...
                        if testtype in triptype:
                            traj_type = traj_type_target_test  # traj_type_target_test  # 独立同分布测试
                        else:
                            traj_type = [0, 1]  # 域外分布测试范围
                        ...
                        for test_traj in test_trajlist:
                            ...
                            if MA_envmode == 'InHiGNSSSatCov_PreCovPos-PosAttCorrectCov':
                                env = DummyVecEnv([lambda: Continuous_PrePosCov_PosAttCorrectCov_InHiGNSSSatCov(config_agent_1, config_agent_2, **test_kwargs)])
