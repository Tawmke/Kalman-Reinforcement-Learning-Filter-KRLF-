...
from .buffers_FT import RecurrentDictRolloutBuffer_FT
...
class RecurrentPPO_FT(OnPolicyAlgorithm):
    """
    ...
    def __init__(
        ...
        self._last_lstm_states_ref = None
    ...
    def _freeze_reference_policy(self):
        """
        请在你 load_state_dict() 之后，learn() 之前调用此方法！
        它会克隆当前被赋好初始值的 policy，并彻底冻结它。
        """
        print("Freezing current policy as the Reference Model...")
        self.ref_policy = deepcopy(self.policy)
        self.ref_policy.eval()  # 切换到评估模式 (关闭 Dropout/BatchNorm 等)

        # 冻结参数，防止占用梯度计算资源，并确保它不被意外更新
    ...
    def _setup_model(self) -> None:
        ...
        buffer_cls = (
            RecurrentDictRolloutBuffer_FT if isinstance(self.observation_space, gym.spaces.Dict) else RecurrentRolloutBuffer
        )
        ...
        self.scheduler = th.optim.lr_scheduler.StepLR(self.policy.optimizer, step_size=1e2, gamma=0.95) # step_size=5e4
        ...
        self._last_lstm_states_ref = RNNStates(
            (
                th.zeros(single_hidden_state_shape).to(self.device),
                th.zeros(single_hidden_state_shape).to(self.device),
            ),
        ...
        # freeze the ref policy
        self._freeze_reference_policy()
    ...
    def collect_rollouts(
        ...
        """
        assert isinstance(
            rollout_buffer, (RecurrentRolloutBuffer, RecurrentDictRolloutBuffer_FT)
        ...
        lstm_states_ref = deepcopy(self._last_lstm_states_ref)
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                _, _, _, lstm_states_ref = self.ref_policy.forward(obs_tensor, lstm_states_ref, episode_starts)
            ...
            rollout_buffer.add(
                ...
                lstm_states_ref = self._last_lstm_states_ref
            )

            self._last_obs = new_obs
            self._last_episode_starts = dones
            self._last_lstm_states = lstm_states
            self._last_lstm_states_ref = lstm_states_ref
    ...
    def train(self) -> None:
        ...
        # check the ref model
        assert self.ref_policy is not None, "You must call .freeze_reference_policy() before .learn()!"
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                # 使用冻结的参考模型推理
                with th.no_grad():
                    _, ref_log_prob, _ = self.ref_policy.evaluate_actions(rollout_data.observations,
                                                                          rollout_data.actions,
                                                                          rollout_data.lstm_states_ref,
                                                                          rollout_data.episode_starts,
                                                                          )

                # 计算 KL 散度 (KL = log(pi) - log(ref))
                kl_div = th.mean(log_prob - ref_log_prob)
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:  #改：self.num_timesteps——>iteration
            ...
            # print(f'{self.logger.dir}') # 打印文件地址名
