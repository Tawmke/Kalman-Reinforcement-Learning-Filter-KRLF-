...
class CustomATF1_GnssHisInnStateCov(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_GnssHisInnStateCov, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["History"].shape[-1]
        dim_input3 = observation_space["innovation"].shape[-1]
        dim_input4 = observation_space["State"].shape[-1]
        dim_input5 = observation_space["Cov"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['History'])
        w3 = self.attention3(observations['innovation'])
        w4 = self.attention4(observations['State'])
        w5 = self.attention5(observations['Cov'])
        out=torch.cat((observations['gnss']*w1,observations['History']*w2,
                       observations['innovation']*w3, observations['State']*w4,
                       observations['Cov']*w5),dim=-1)
        return out

class CustomATF1_GnssHisInnPosP_A2(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_GnssHisInnPosP_A2, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["History"].shape[-1]
        dim_input3 = observation_space["innovation"].shape[-1]
        dim_input4 = observation_space["State"].shape[-1]
    ...
    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        # 出于规范问题，和双A环境和单A的观测命名不一致
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['History'])
        w3 = self.attention3(observations['innova_cor'])
        w4 = self.attention4(observations['POS'])
        w5 = self.attention5(observations['KF_P'])
        out=torch.cat((observations['gnss']*w1,observations['History']*w2,
                       observations['innova_cor']*w3, observations['POS']*w4,
                       observations['KF_P']*w5),dim=-1)
        return out

class CustomATF1_GnssHisInnPosAttP_A2(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_GnssHisInnPosAttP_A2, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["History"].shape[-1]
        dim_input3 = observation_space["innovation"].shape[-1]
        dim_input4 = observation_space["State"].shape[-1]
        ...
        self.attention4 = Attention1(dim_input4)
        self.attention5 = Attention1(dim_input5)
        self.pos_num = dim_input2

    def forward(self, observations: torch.Tensor) -> torch.Tensor:
        # 出于规范问题，和双A环境和单A的观测命名不一致
        w1 = self.attention1(observations['gnss'])
        w2 = self.attention2(observations['History'][:,:,:self.pos_num])
        w3 = self.attention3(observations['innova_cor'])
        w4 = self.attention4(observations['State'])
        w5 = self.attention5(observations['KF_P'])
        out=torch.cat((observations['gnss']*w1,observations['History'][:,:,:self.pos_num]*w2,
                       observations['innova_cor']*w3, observations['State']*w4,
                       observations['KF_P']*w5),dim=-1)
        return out

class CustomATF1_GnssHisInnPosAttP(BaseFeaturesExtractor):
    """
    用于联合训练用
    """
    def __init__(self, observation_space: gym.spaces.Box, features_dim: int = 256): #注意力自适应权重
        super(CustomATF1_GnssHisInnPosAttP, self).__init__(observation_space, features_dim)
        dim_input1 = observation_space["gnss"].shape[-1]
        dim_input2 = observation_space["History"].shape[-1]
        dim_input3 = observation_space["innova_cor"].shape[-1]
        dim_input4 = observation_space["State"].shape[-1]
        dim_input5 = observation_space["KF_P"].shape[-1]
        self.attention1 = Attention1(dim_input1)
        self.attention2 = Attention1(dim_input2)
        self.attention3 = Attention1(dim_input3)
        self.attention4 = Attention1(dim_input4)
